...
String page_title = type + " Cached Edit";
...
if(entity == null) {
    response.sendRedirect("entity_cached_list.jsp?type=" + type);
    return;
}
...
<script>
            ...
            function deleteEntity() {
                bootbox.confirm("Are you sure to delete this <%=type%>?", function(result){
                    if(result) {
                        var id = $('#id').val();
                        $.get(getAPIURL("api/entity/delete_cache?type=<%=type%>&id=" + id), function(data){
