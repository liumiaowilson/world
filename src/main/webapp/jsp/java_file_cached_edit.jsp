...
<%
String page_title = "Java File Cached Edit";
%>
...
java_file = JavaFileManager.getInstance().getCachedJavaFiles().get(id);
if(java_file == null) {
    response.sendRedirect("java_file_cached_list.jsp");
    return;
}
%>
...
<script>
            ...
            function deleteJavaFile() {
                bootbox.confirm("Are you sure to delete this cached java file?", function(result){
                    if(result) {
                        var id = $('#id').val();
                        $.get(getAPIURL("api/java_file/delete_cache?id=" + id), function(data){
