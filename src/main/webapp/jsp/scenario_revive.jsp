<%
String page_title = "Scenario Revive";
%>
...
<div class="panel panel-default">
    <div class="panel-heading">
        <h3 class="panel-title">Scenario Revive (from: <%=scenario.name%>)</h3>
    </div>
...
$(document).ready(function(){
    ...
    $('#right_btn').click(function(){
        $.post(getAPIURL("api/scenario/revive"), { id: <%=scenario.id%>, description: $('#description').val() }, function(data) {
            var status = data.result.status;
            var msg = data.result.message;
            if("OK" == status) {
                showSuccess(msg);
                jumpTo("scenario_react.jsp?id=<%=scenario.id%>");
            }
            else {
                showDanger(msg);
            }
