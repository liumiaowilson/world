...
<script>
            function deleteTaskAttrRule() {
                bootbox.confirm("Are you sure to delete this task attr rule?", function(result){
                    if(result) {
                        var id = $('#id').val();
                        $.get(getAPIURL("api/task_attr_rule/delete?id=" + id), function(data){
            ...
            $(document).ready(function(){
                ...
                $('#form').validator().on('submit', function (e) {
                    if (e.isDefaultPrevented()) {
                        ...
                        l.ladda('start');
                        $.post(getAPIURL("api/task_attr_rule/update"), { id: $('#id').val(), name: $('#name').val(), policy: $('#policy').val(), priority: $('#priority').val(), impl: $('#impl').val()}, function(data) {
                            var status = data.result.status;
                            var msg = data.result.message;
                            if("OK" == status) {
                                showSuccess(msg);
