...
<form id="form" data-toggle="validator" role="form">
    ...
    <fieldset class="form-group">
        ...
        <textarea class="form-control" id="description" rows="5" maxlength="200" placeholder="Enter detailed description" required><%=context.description%></textarea>
    </fieldset>
...
<script>
            function deleteContext() {
                bootbox.confirm("Are you sure to delete this context?", function(result){
                    if(result) {
                        var id = $('#id').val();
                        $.get(getAPIURL("api/context/delete?id=" + id), function(data){
            ...
            $(document).ready(function(){
                ...
                $('#form').validator().on('submit', function (e) {
                    if (e.isDefaultPrevented()) {
                        ...
                        l.ladda('start');
                        $.post(getAPIURL("api/context/update"), { id: $('#id').val(), name: $('#name').val(), description: $('#description').val(), color: $('#color').val()}, function(data) {
                            var status = data.result.status;
                            var msg = data.result.message;
                            if("OK" == status) {
                                showSuccess(msg);
