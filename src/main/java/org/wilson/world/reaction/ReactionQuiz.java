...
public class ReactionQuiz extends SystemQuiz {
    public ReactionQuiz() {
        this.setName("Reaction Quiz");
        this.setDescription("A quiz for testing the reactions");
    }
    ...
    public QuizResult process(QuizPaper paper) {
        ...
        if(ratio >= 0.6) {
            ...
            NotifyManager.getInstance().notifySuccess("Gained one extra experience because of doing reaction quiz");
        }
        
        Event event = new Event();
        event.type = EventType.DoReactionQuiz;
    ...
    public void init() {
        ...
        int maxLength = ConfigManager.getInstance().getConfigAsInt("reaction.quiz.max_length", 5);
        List<QuizPair> pairs = ReactionManager.getInstance().getReactionQuizPairs();
