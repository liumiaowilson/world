...
@Path("zodiac_sign")
public class ZodiacSignAPI {
    private static final Logger logger = Logger.getLogger(ZodiacSignAPI.class);
    ...
    public Response get(
        ...
        try {
            ZodiacSign sign = ZodiacSignManager.getInstance().getZodiacSign(id);
            if(sign != null) {
                APIResult result = APIResultUtils.buildOKAPIResult("ZodiacSign has been successfully fetched.");
                result.data = sign;
                return APIResultUtils.buildJSONResponse(result);
            }
            else {
                return APIResultUtils.buildJSONResponse(APIResultUtils.buildErrorAPIResult("ZodiacSign does not exist."));
            }
        }
        catch(Exception e) {
    ...
    public Response list(
        ...
        try {
            List<ZodiacSign> signs = ZodiacSignManager.getInstance().getZodiacSigns();
            
            Collections.sort(signs, new Comparator<ZodiacSign>() {

                @Override
                public int compare(ZodiacSign o1, ZodiacSign o2) {
                    return o1.name.compareTo(o2.name);
                }
                
            });
            
            APIResult result = APIResultUtils.buildOKAPIResult("ZodiacSign have been successfully fetched.");
            result.list = signs;
            return APIResultUtils.buildJSONResponse(result);
        }
        catch(Exception e) {
