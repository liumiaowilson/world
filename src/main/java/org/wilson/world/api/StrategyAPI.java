...
public class StrategyAPI {
    private static final Logger logger = Logger.getLogger(StrategyAPI.class);
    ...
    public Response get(
        ...
        try {
            Strategy strategy = StrategyManager.getInstance().getStrategy(id);
            if(strategy != null) {
                APIResult result = APIResultUtils.buildOKAPIResult("Strategy has been successfully fetched.");
                result.data = strategy;
                return APIResultUtils.buildJSONResponse(result);
            }
            else {
                return APIResultUtils.buildJSONResponse(APIResultUtils.buildErrorAPIResult("Strategy does not exist."));
            }
        }
        catch(Exception e) {
    ...
    public Response list(
        ...
        try {
            List<Strategy> strategies = StrategyManager.getInstance().getStrategies();
            ...
            APIResult result = APIResultUtils.buildOKAPIResult("Strategies have been successfully fetched.");
            result.list = strategies;
            return APIResultUtils.buildJSONResponse(result);
        }
        catch(Exception e) {
            logger.error("failed to get strategies", e);
            return APIResultUtils.buildJSONResponse(APIResultUtils.buildErrorAPIResult(e.getMessage()));
        }
    }
    ...
    public Response train(
        ...
        try {
            if(!StringUtils.isBlank(solution)) {
                ExpManager.getInstance().train(solution, "Gained an extra experience point from training strategy.");
            }
            
            Event event = new Event();
            event.type = EventType.TrainStrategy;
            EventManager.getInstance().fireEvent(event);
            
            APIResult result = APIResultUtils.buildOKAPIResult("Strategy has been successfully trained.");
            return APIResultUtils.buildJSONResponse(result);
        }
        catch(Exception e) {
            logger.error("failed to train strategy", e);
            return APIResultUtils.buildJSONResponse(APIResultUtils.buildErrorAPIResult(e.getMessage()));
        }
    }
}
