...
public class BeautyAPI {
    ...
    public Response clean(
        ...
        try {
            BeautyManager.getInstance().getBeautiesRemoved().clear();
            APIResult result = APIResultUtils.buildOKAPIResult("Removed beauties have been successfully cleaned.");
            return APIResultUtils.buildJSONResponse(result);
        }
        catch(Exception e) {
            logger.error("failed to clean removed beauties!", e);
            return APIResultUtils.buildJSONResponse(APIResultUtils.buildErrorAPIResult(e.getMessage()));
        }
    }
}
